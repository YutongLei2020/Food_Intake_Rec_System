...
def calculate_rec_restaurant():
    ...
    #print(user_db[0].keys(), len(user_db), user_db[1].keys(), user_db[-1].keys())
...
def calculate_calories():
    with open('CS_125_database.json', 'r') as file:
            json_data = file.read()
    restaurents = json.loads(json_data)
...
def feedback():
    ...
    if request.method == 'POST':
        ...
        new_user_db = json.dumps(user_db, default=set_default)
        with open('userdata.json', 'w') as file:
                file.write(new_user_db)


    return jsonify({'received': 'true'})

@app.route('/LoadGoal', methods=['GET', 'OPTIONS'])
def LoadGoal():
    with open('userdata.json', 'r') as file:
            json_data = file.read()
    user_db = json.loads(json_data)
    ...
    for i in user_db:
        if i['email'] == email:
            user = i
    return jsonify(user['return_calories'])
...
def LoadUserData(): 
    ...
    restraurant = json.loads(json_data2)
    if request.method == 'POST':
        # print(request)
        with open('userdata.json', 'r') as file:
            json_data = file.read()
...
# ]
        ...
        #print(data)
        users.append(data)
        fileInfo = json.dumps(users, default=set_default)
        with open('userdata.json', 'w') as file:
             file.write(fileInfo)
